 ...
 * optional datum/tgui/master_ui The parent UI.
 * optional datum/ui_state/state The state used to determine status.
 */
...
/datum/proc/update_static_data_for_all_viewers()
	for (var/datum/tgui/window as anything in SStgui.open_uis_by_src[src.UID()])
...
/datum/proc/ui_state(mob/user)
	CRASH("Paradise currently uses the `state` parameter to ui_interact instead of this.")
